...
builder.Services.AddAuthorization(options =>
{
    options.AddPolicy("CanReadHr", policy =>
    {
        ...
        policy.RequireClaim("permission", PermissionConstants.HrRead);
    });

    options.AddPolicy("CanReadInventory", policy =>
    {
        policy.RequireAuthenticatedUser();
        policy.RequireClaim("permission", PermissionConstants.InventoryRead);
    });

    options.AddPolicy("CanWriteInventory", policy =>
    {
        policy.RequireAuthenticatedUser();
        policy.RequireClaim("permission", PermissionConstants.InventoryWrite);
    });

    options.AddPolicy("CanWriteHr", policy =>
    {
        policy.RequireAuthenticatedUser();
        policy.RequireClaim("permission", PermissionConstants.HrWrite);
    });

    options.AddPolicy("CanManageIdentity", policy =>
    {
        policy.RequireAuthenticatedUser();
        policy.RequireClaim("permission", PermissionConstants.IdentityRolesManage);
    });
});
